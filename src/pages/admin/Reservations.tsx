...
const Reservations = () => {
    ...
    const openApprovalModal = async (reservation: any) => {
        try {
            ...
            const locationTables = tablesSnap.docs
                .map(d => ({ id: d.id, ...d.data() } as any))
                .filter(t => t.location === reservation.location);

            setAvailableTables(locationTables);
    ...
    const handleApprove = async () => {
        ...
        try {
            ...
            // Trigger Email Simulation instantly by setting it for the next modal
    ...
    const handleUpdateStatus = async (id: string, newStatus: string) => {
        try {
            ...
            // Sync Table Status if applicable
            ...
            if (res && res.tableId) {
                ...
                if (newStatus === "active" || newStatus === "arrived") {
                    ...
                    await updateDoc(tableRef, { status: "cleaning" });
                }
            }
        ...
        <div className="min-h-screen bg-background text-foreground flex flex-col md:flex-row">
            ...
            <aside className="w-full md:w-64 border-b md:border-r border-border bg-card flex flex-col">
                ...
                <nav className="flex-1 px-4 space-y-2 pb-4 md:pb-0">
                    ...
                    <a href="/admin/reservations" className="flex items-center gap-3 px-4 py-2 bg-primary text-primary-foreground rounded-lg transition-colors text-sm">
                    ...
                    <a href="/admin/scan" className="flex items-center gap-3 px-4 py-2 text-muted-foreground hover:bg-muted hover:text-foreground rounded-lg transition-colors text-sm">
            ...
            <main className="flex-1 p-4 md:p-8 space-y-8 overflow-auto">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">Manage and approve booking requests.</p>
                    </div>
                </div>
                                ...
                                filteredReservations.map((res) => (
                                    <div key={res.id} className="group p-4 bg-background rounded-xl border border-border hover:border-primary/20 transition-all duration-300">
                                        <div className="flex flex-col gap-4">
                                            ...
                                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 bg-muted/20 p-3 rounded-lg border border-border/50">
                                            ...
                                            {res.tableMarking && (
                                                <div className="flex items-center gap-2 px-3 py-1.5 bg-primary/5 border border-primary/10 rounded-md w-fit">
                                                    <span className="text-[10px] font-bold text-primary uppercase">Table: {res.tableMarking}</span>
                                                </div>
                                            )}
                                            ...
                                            <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-border/50 mt-1">
                                                {res.status === "pending" && (
                                                    <Button
                                                        ...
                                                        className="h-8 bg-accent hover:bg-accent/90 text-accent-foreground gap-2 flex-1 sm:flex-none text-[10px] px-3 transition-all transform hover:scale-105"
                                                        size="sm"
                                                    >
                                                ...
                                                {res.status === "approved" && (
                                                    <Button
                                                        ...
                                                        className="h-8 bg-sage hover:bg-sage/90 text-white gap-2 flex-1 sm:flex-none text-[10px] px-3"
                                                        size="sm"
                                                    >
                                                ...
                                                {["approved", "arrived", "active"].includes(res.status) && (
                                                    <select
                                                        className="h-8 bg-background border border-border rounded-md px-2 text-[10px] focus:ring-1 focus:ring-accent outline-none font-medium"
                                                        value={res.status}
                                                ...
                                                {res.status !== "cancelled" && res.status !== "completed" && (
                                                    <Button
                                                        ...
                                                        className="h-8 text-muted-foreground hover:text-destructive hover:bg-destructive/5 gap-2 text-[10px] px-2"
                                                        size="sm"
                                                    >
